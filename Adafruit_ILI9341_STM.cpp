...
void Adafruit_ILI9341_STM::pushColor2x2( uint16_t * p, uint16_t x0, uint16_t y0, const uint16_t w, const uint16_t h) {
  ...
  *csport &= ~cspinmask;
  ...
  for ( int y= 0; y < h; y++ ) {
		...
		// line processing
		do {
			if ( *p1 != 0 && prev != 0) {			// ha nem átlátszó részben van, volt
				bend = x;	
			}
			if ( *p1 != 0 && prev==0 ) {
				bstart = x;            				// nem átlátszó block kezdődik.
			}
		    if ( *p1 == 0 && prev != 0 || x == w-1 ) {	 
				// ha véget ért egy átlátszó rész
				if ( prev != 0 )  {
					if ( x == w-1 )	bend = w;
					setAddrWindow ( x0 + bstart, y0+y, x0+bstart + ( bend-bstart ) , y0 +y+1 );
				
					*dcport |=  dcpinmask;
					*csport &= ~cspinmask;		
					SPI.setDataSize(SPI_CR1_DFF);
					SPI.dmaSend ( p + y *w + bstart, ( bend-bstart ) );	
					*csport |= cspinmask;
				}
			}
		/*	if ( *p1==0 && prev == 0 ) {				
			    drawPixel ( x0+x, y0 +y, 0x5000 );
			}	*/
			prev = *p1;
			x++;
			p1++;
...
void Adafruit_ILI9341_STM::pushLine ( uint16_t * linedata, uint16_t width ) {
    SPI.setDataSize (SPI_CR1_DFF); // Set SPI 16bit mode
    *dcport |=  dcpinmask;
    *csport &= ~cspinmask;
    SPI.dmaSend( linedata , width );
    *csport |= cspinmask;
    SPI.setDataSize (0);
}

void Adafruit_ILI9341_STM::drawBMP(const uint8_t * icon, int16_t x, int16_t y, int16_t width, int16_t height, uint16_t * colors)
{
  setAddrWindow(x, y, x + width *2 - 1, y + height *2 - 1);
  uint16_t tempbuffer[width*2];  
  SPI.setDataSize (SPI_CR1_DFF); // Set SPI 16bit mode
  for (int yaxis = 0; yaxis < height; yaxis++)
  {

    uint8_t color, c2;
	uint16_t yoff =  yaxis * width;
    for (uint16_t xaxis = 0; xaxis < width; xaxis++)
    {
	   //color = 0b0000000011100000;
		c2 = icon[yoff+ xaxis];
		//color = c2 | ( c2 << 11);
		//color |= 0b0000000011100000;
		//c2%= 64;
		tempbuffer[xaxis*2] =   colors[c2];
		tempbuffer[xaxis*2+1] = colors[c2];
    }

	*dcport |=  dcpinmask;
	*csport &= ~cspinmask;
    SPI.dmaSend( tempbuffer , width*2 );
    *csport |= cspinmask;
